...
shinygenui implements declarative, catalog-constrained generative UI for Shiny.
Read `DESIGN.md` first; it is the architecture spec of record and must be
...
- `R6::R6Class` requires unique names across `public` + `private` + `active` combined.
